...
let isubst_prog glob prog =
  ...
  let isubst_glob (x, gd) = 
    ...
    let x = 
      let x = 
        gsubst_gvar subst_v {gv = L.mk_loc L._dummy x; gk = Expr.GKglob} in
...
let remove_params (prog : 'info pprog) =
  ...
  let mk_word ws e = 
    Word0.wrepr ws (Prog.z_of_bi (clamp ws (constant_of_expr e))) in
  let doglob (x, e) = 
    match x.v_ty, e with
    | Bty (U ws), GEword e ->
      x, Expr.Gword (ws, mk_word ws e) 
    | Arr(ws,n), GEarray es ->
      assert (List.length es = n);
      let p = Prog.pos_of_int (n * size_of_ws ws) in
      let t = ref (Warray.WArray.empty p) in
      let doit i e = 
        match Warray.WArray.set p ws !t (Prog.z_of_int i) (mk_word ws e) with
        | Ok t1 -> t := t1
        | _ -> assert false in
      List.iteri doit es;
      x, Expr.Garr(p, !t)
