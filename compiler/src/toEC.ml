...
(*    let pp_init fmt sz = 
  ...
  | Pget(aa, ws, x, e) -> 
    assert (aa = Warray.AAscale); (* Other not implemented *)
...
let pp_lval1 env pp_e fmt (lv, (ety, e)) = 
  ...
  | Laset (aa, ws, x,e1) -> 
    assert (aa=Warray.AAscale); (* Other not yet implemented *)
...
module Leak = struct 
  ...
  let rec safe_e_rec env safe = function
    ...
    | Pget (a, ws, x, e) -> 
      assert (a = Warray.AAscale); (* Not yet implemented *)
      let x = x.gv in
      let safe = 
        let (_s,option) = Mv.find (L.unloc x) env.vars in
  ...
  let safe_lval env = function
    ...
    | Laset(aa, ws, x,e) -> 
      assert (aa = Warray.AAscale); (* Not yet implemented *)
...
let pp_glob_decl env fmt (x,d) =
  match Prog.glob_of_cglob x.v_ty d with
  | `GWord(ws, z) -> 
    Format.fprintf fmt "@[abbrev %a = %a.of_int %a.@]@ "
      (pp_var env) x pp_Tsz ws B.pp_print z
  | `GArray(wz, t) ->
    let pp_elem fmt z = 
